...
import { addMessage, fetchMessages, fetchConversations, markMessagesAsRead, updateUserStatus, clearMessages, incrementUnread, resetUnread, resetAllUnread, handleReadMessage, updateMessage, setCurrentUserId, setMessagesRead, setActiveChatId, deleteMessageLocally } from "../store/slices/messageSlice";
...
const ChatWidget = () => {
  ...
  const { messages, conversations, loading } = useSelector((state) => state.messages);
  ...
  return (
    <>
      ...
      {/* Chat Window */}
      {isOpen && (
        <div className="w-[380px] sm:w-[420px] h-[580px] bg-white dark:bg-gray-900 rounded-3xl shadow-2xl border border-gray-100 dark:border-gray-800 flex flex-col overflow-hidden animate-in slide-in-from-bottom-5 duration-300">
                ...
                {conversations.length === 0 ? (
                  ...
                  conversations.map((conv) => (
                    <button
                      ...
                      className="w-full flex items-center gap-3 p-3 rounded-2xl bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 hover:border-black dark:hover:border-white transition-all text-left shadow-sm group"
                    >
                      <div className="relative">
                        <div className="w-11 h-11 rounded-xl bg-gray-100 dark:bg-gray-700 flex items-center justify-center overflow-hidden border border-gray-200 dark:border-gray-600">
                        ...
                        {conv.userInfo.isOnline && (
                          <span className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full bg-green-500 border-2 border-white dark:border-gray-800" />
                        )}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-start mb-0.5">
                          <h4 className="font-bold text-gray-900 dark:text-gray-100 text-sm truncate">{conv.userInfo?.name || "Unknown User"}</h4>
                          <span className="text-[10px] font-bold text-gray-400">
                            {conv.lastMessageTime ? format(new Date(conv.lastMessageTime), "p") : ""}
                          </span>
                        </div>
                        <p className="text-xs text-gray-500 dark:text-gray-400 truncate font-semibold">{conv.lastMessage || "No messages yet"}</p>
                      </div>
                      {conv.unreadCount > 0 && (
              ...
              <>
                {messages.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-center px-6">
                    ...
                    <h4 className="font-bold text-gray-900 dark:text-white mb-1">Start a conversation</h4>
                  ...
                  messages
                    .filter(m => String(m.sender) === String(activeChat?._id) || String(m.receiver) === String(activeChat?._id))
                    .map((msg, i) => {
                    ...
                    return (
                      <div
                        key={msg._id || i}
                        className={`flex flex-col ${isOwn ? "items-end" : "items-start"} animate-in fade-in-0 slide-in-from-bottom-2 duration-300`}
                        style={{ animationDelay: `${Math.min(i * 20, 300)}ms`, animationFillMode: "both" }}
                      >
                        {/* Row: bubble + action buttons side by side */}
                        <div className={`flex items-center gap-1.5 group ${isOwn ? "flex-row-reverse" : "flex-row"} max-w-full`}>

                          {/* Message Bubble */}
                          <div className={`max-w-[75%] sm:max-w-[80%] px-4 py-2.5 rounded-2xl text-sm font-semibold shadow-sm transition-all duration-200 ${
                            msg.deletedForEveryone
                              ? "bg-gray-100 dark:bg-gray-800 text-gray-400 italic border border-dashed border-gray-300 dark:border-gray-600 opacity-70"
                              : isOwn
                                ? "bg-black text-white rounded-br-none hover:shadow-md"
                                : "bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 text-gray-800 dark:text-gray-200 rounded-bl-none hover:shadow-md"
                          }`}>
                            {msg.deletedForEveryone ? (
                              ...
                              <>
                                {msg.messageType === "image" ? (
                                  ...
                                  <div className="flex items-center gap-3 py-1">
                                    <div className={`p-2 rounded-xl ${isOwn ? "bg-gray-800" : "bg-gray-100 dark:bg-gray-700"} flex items-center justify-center shrink-0 transition-transform duration-200 group-hover:scale-105`}>
                            ...
                            <div className="flex flex-col gap-1 opacity-0 group-hover:opacity-100 scale-90 group-hover:scale-100 transition-all duration-200 shrink-0">
                              {canUserEdit && msg.messageType !== "image" && msg.messageType !== "file" && (
                                <button
                                  ...
                                  className="p-1.5 rounded-lg bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-white hover:scale-110 active:scale-95 transition-all duration-150 shadow-sm"
                                  title="Edit message"
                                >
                                  <Pencil size={12} />
                                </button>
          ...
          {(!isAdmin || activeChat) && (
            <form onSubmit={handleSendMessage} className="p-4 bg-white dark:bg-gray-900 border-t border-gray-100 dark:border-gray-800">
              <div className="flex items-center gap-2 bg-gray-100 dark:bg-gray-800 rounded-2xl px-3 py-2 border border-transparent focus-within:border-black dark:focus-within:border-white transition-all">
                <input
                  type="text"
                ...
                <div className="flex items-center gap-1">
                  <button 
                    ...
                    className={`p-1.5 text-gray-400 hover:text-black dark:hover:text-white transition-colors ${uploadingImage ? 'animate-pulse' : ''}`}
                    title="Send Image"
                  >
                    <ImageIcon size={20} />
                  </button>
                ...
                <button type="submit" disabled={!message.trim()} className="text-black dark:text-white hover:scale-110 disabled:opacity-30 disabled:hover:scale-100 transition-transform">
                  <Send size={20} />
                </button>
              </div>
            </form>
    ...
    {activeMenu && activeMenuData && (
      <div
        ...
        className="fixed z-[9999] bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-2xl min-w-[190px] overflow-hidden animate-in fade-in-0 zoom-in-95 duration-150"
        onClick={(e) => e.stopPropagation()}
      >
        {confirmDelete ? (
          ...
          <div className="px-4 py-3 animate-in fade-in-0 slide-in-from-top-1 duration-200">
            ...
            <p className="text-[11px] text-gray-500 dark:text-gray-400 mb-3 leading-snug pl-8">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="flex-1 py-1.5 rounded-lg bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 active:scale-95 text-[11px] font-bold transition-all duration-150"
              >
                Cancel
              </button>
            </div>
          ...
          <>
            <button
              ...
              className="w-full px-4 py-2.5 text-left text-[12px] font-semibold text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 active:bg-gray-100 flex items-center gap-2.5 transition-colors duration-100"
            >
            ...
            {activeMenuData.isOwn && (new Date() - new Date(activeMenuData.createdAt)) / (1000 * 60) < 15 && (
              <button
                ...
                className="w-full px-4 py-2.5 text-left text-[12px] font-semibold text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 active:bg-red-100 flex items-center gap-2.5 border-t border-gray-100 dark:border-gray-800 transition-colors duration-100"
              >
                <Trash2 size={13} />
                Delete for Everyone
              </button>
